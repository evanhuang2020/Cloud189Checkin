...
const pushBarksent = require("./push/pushBark");
...
const pushBarksent = (title, desp) => {
  ...
  superagent
    .post(`https://api.day.app/${Bark.sendKey}.send`)
    .type("form")
    .send(data)
    .end((err, res) => {
      if (err) {
        logger.error(`Bark推送失败:${JSON.stringify(err)}`);
        return;
      }
      const json = JSON.parse(res.text);
      if (json.code !== 0) {
        logger.error(`Bark推送失败:${JSON.stringify(json)}`);
      } else {
